...
function handleLogin(e) {
    ...
    if (inputId === savedId && inputPass === savedPass) {
        ...
        document.getElementById('login-screen').classList.add('hidden');
        document.getElementById('app-wrapper').classList.remove('hidden');
        checkUrlForTransfer(); // ログイン直後にURL転送を確認
    } else {
        alert("IDまたはパスワードが間違っています。");
    }
}
function handleEmergencyReset() {
    if(confirm("IDとパスワードを (admin / admin) に戻しますか？")) {
        ...
        alert("リセット完了。 admin / admin でログインしてください。");
    }
}
...
function addNewGirl() {
    const input = document.getElementById('new-girl-name'); const name = input.value.trim();
    if (name && !girlsData.includes(name)) { girlsData.push(name); girlsData.sort((a, b) => a.localeCompare(b, 'ja')); input.value = ''; renderGirls(); }
}
...
function updateSummary() {
    ...
    document.getElementById('today-count').textContent = todayCount; document.getElementById('today-sales').textContent = todaySales.toLocaleString();
    document.getElementById('total-count').textContent = totalCount; document.getElementById('total-sales').textContent = totalSales.toLocaleString();
}
...
function updateLineMessagePreview() {
    ...
    let selectedOps = []; document.querySelectorAll('.op-checkbox:checked').forEach(cb => selectedOps.push(cb.value));

    let nomStr = {'F': 'フリー', 'N': 'ネット指名', '本': '本指名'}[nominationClass] || "";
    if (nominationClass === '本' && prevVisit) nomStr += `(前回${prevVisit})`;

    let custStr = (custClass === '新') ? (custName ? `新規${custName}様` : "新規様") : (custName ? `会員${custName}様` : "会員様");

    let placeLine = "";
    if (meetingPlace !== 'その他') placeLine = meetingPlace.endsWith("待ち合わせ") ? `${meetingPlace}\n` : `${meetingPlace}待ち合わせ\n`;
...
function exportTransferData() {
    ...
    const code = btoa(encodeURIComponent(JSON.stringify(data))); // データを暗号化文字列に変換
...
function checkUrlForTransfer() {
    ...
    if (code) {
        ...
        window.history.replaceState({}, document.title, window.location.pathname); // URLを綺麗に戻す
    }
}
...
function handleFormSubmit(e) {
    ...
    let hotelAbbrev = "";
    if (meetingPlace !== 'その他' && hotelSelect !== 'その他') hotelAbbrev = HOTEL_ABBREV_MAPPING[hotelSelect] || "";
